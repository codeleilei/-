...
3、栈：
	数据结构：	一个大的栈的class
				一个实际栈struct（一个数组 一个栈顶指针）
	操作总结：
		最重要的是栈顶指针 我们初始化栈顶指针为-1  （所以判断栈是否为空的依据就是判断top指针是否为-1）
		top定义为-1的原因：
			1、同步c语言数组从0开始的特性（这个和线性表不一样 线性表要依着线性表来---就是写代码时逻辑上是从1开始）
			   比如第一个元素进入的时候 top=0  正好对应栈数组的a[0]
